@testset "RSparseBitSet" begin
    ...
    @testset "clearMask!" begin
        ...
        @test set.mask[1:6] .= UInt64(0)
        @test set.mask[7:end] .= typemax(UInt64)
    end
    ...
    @testset "reverseMask!" begin
        ...
        @test set.mask[1:3] .= UInt64(0)
        @test set.mask[3:6] .= typemax(UInt64)
        @test set.mask[7:end] .= UInt64(0)
    end
    ...
    @testset "intersectWithMask!" begin
        ...
        @test set[6]
        @test set[12]
        @test !any(set[setdiff(1:24, [1,6,12])])
    end
