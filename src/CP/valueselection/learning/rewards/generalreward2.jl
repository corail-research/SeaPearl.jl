...
function GeneralReward2(model::CPModel)
    ...
    if !isnothing(model.objective)
        return GeneralReward2(0, model.objective.domain.min.value, model.objective.domain.max.value, nothing, 0.0, 2.0)
    else
        return GeneralReward2(0, nothing, nothing, nothing, 0.0, 2.0)
    end
end

"""
    set_reward!(::StepPhase, lh::LearnedHeuristic{GeneralReward2, A}, model::CPModel, symbol::Union{Nothing, Symbol})

Change the "current_reward" attribute of the LearnedHeuristic at the StepPhase.
"""
function set_reward!(::Type{StepPhase}, lh::LearnedHeuristic{SR, GeneralReward2, A}, model::CPModel, symbol::Union{Nothing, Symbol}) where {
...
"""
    set_reward!(::DecisionPhase, lh::LearnedHeuristic{GeneralReward2, O}, model::CPModel)
...
"""
function set_reward!(::Type{DecisionPhase}, lh::LearnedHeuristic{SR, GeneralReward2, A}, model::CPModel) where {
    SR <: AbstractStateRepresentation,
    A <: ActionOutput
}
...
"""
    set_reward!(::EndingPhase, lh::LearnedHeuristic{GeneralReward2, A}, model::CPModel, symbol::Union{Nothing, Symbol})
...
"""
function set_reward!(::Type{EndingPhase}, lh::LearnedHeuristic{SR, GeneralReward2, A}, model::CPModel, symbol::Union{Nothing, Symbol}) where {
    SR <: AbstractStateRepresentation, 
    A <: ActionOutput
}
