export CPEpsilonGreedyExplorer, CPGreedyExplorer
...
"""
mutable struct CPEpsilonGreedyExplorer{Kind,IsBreakTie,R} <: RL.AbstractExplorer
    ϵ_stable::Float64
    ϵ_init::Float64
    warmup_steps::Int
    decay_steps::Int
...
function CPEpsilonGreedyExplorer(;
    ϵ_stable,
    kind = :linear,
    ϵ_init = 1.0,
    warmup_steps = 0,
    ...
    CPEpsilonGreedyExplorer{kind,is_break_tie,typeof(rng)}(
        ϵ_stable,
        ϵ_init,
        warmup_steps,
        decay_steps,
...
Flux.testmode!(p::CPEpsilonGreedyExplorer, mode = true) = p.is_training = !mode

CPEpsilonGreedyExplorer(ϵ; kwargs...) = CPEpsilonGreedyExplorer(; ϵ_stable = ϵ, kwargs...)

function get_ϵ(s::CPEpsilonGreedyExplorer{:linear}, step)
...
function get_ϵ(s::CPEpsilonGreedyExplorer{:exp}, step)
    if step <= s.warmup_steps
        s.ϵ_init
    else
...
get_ϵ(s::CPEpsilonGreedyExplorer) = s.is_training ? get_ϵ(s, s.step) : 0.0
...
"""
function (s::CPEpsilonGreedyExplorer{<:Any,true})(values)
    ϵ = get_ϵ(s)
    s.is_training && (s.step += 1)
    rand(s.rng) >= ϵ ? rand(s.rng, find_all_max(values)[2]) : rand(s.rng, 1:length(values))
end

function (s::CPEpsilonGreedyExplorer{<:Any,false})(values)
    ϵ = get_ϵ(s)
    s.is_training && (s.step += 1)
    rand(s.rng) >= ϵ ? findmax(values)[2] : rand(s.rng, 1:length(values))
end

function (s::CPEpsilonGreedyExplorer{<:Any,true})(values, mask)
    ϵ = get_ϵ(s)
    s.is_training && (s.step += 1)
...
function (s::CPEpsilonGreedyExplorer{<:Any,false})(values, mask)
    ϵ = get_ϵ(s)
    s.is_training && (s.step += 1)
...
"""
function RLBase.get_prob(s::CPEpsilonGreedyExplorer{<:Any,true}, values)
...
function RLBase.get_prob(s::CPEpsilonGreedyExplorer{<:Any,true}, values, action::Integer)
    ϵ, n = get_ϵ(s), length(values)
    max_val_inds = find_all_max(values)[2]
...
function RLBase.get_prob(s::CPEpsilonGreedyExplorer{<:Any,false}, values)
    ϵ, n = get_ϵ(s), length(values)
    probs = fill(ϵ / n, n)
...
function RLBase.get_prob(s::CPEpsilonGreedyExplorer{<:Any,false}, values, action::Integer)
    ϵ, n = get_ϵ(s), length(values)
...
function RLBase.get_prob(s::CPEpsilonGreedyExplorer{<:Any,true}, values, mask)
    ϵ, n = get_ϵ(s), length(values)
    probs = zeros(n)
...
function RLBase.get_prob(s::CPEpsilonGreedyExplorer{<:Any,false}, values, mask)
    ϵ, n = get_ϵ(s), length(values)
    probs = zeros(n)
    probs[mask] .= ϵ / sum(mask)
...
RLBase.reset!(s::CPEpsilonGreedyExplorer) = s.step = 1

# Though we can achieve the same goal by setting the ϵ of [`EpsilonGreedyExplorer`](@ref) to 0,
# the GreedyExplorer is much faster.
struct CPGreedyExplorer <: RL.AbstractExplorer end

(s::CPGreedyExplorer)(values) = findmax(values)[2]
(s::CPGreedyExplorer)(values, mask) = findmax(values, mask)[2]

function RLBase.get_prob(s::CPGreedyExplorer, values)
    prob = zeros(length(values))
    prob[findmax(values)[2]] = 1.0
    Categorical(prob)
end

RLBase.get_prob(s::CPGreedyExplorer, values, action::Integer) =
    findmax(values)[2] == action ? 1.0 : 0.0

function RLBase.get_prob(s::CPGreedyExplorer, values, mask)
    prob = zeros(length(values))
