...
"""
Base.@kwdef struct HeterogeneousFFCPNNv4 <: NNStructure
    graphChain
    varChain::Flux.Chain = Flux.Chain()
    valChain::Flux.Chain = Flux.Chain()
    conChain::Flux.Chain = Flux.Chain()
    ...
    function HeterogeneousFFCPNNv4(graphChain, nodeChain::Flux.Chain = Flux.Chain(), globalChain::Flux.Chain = Flux.Chain(), outputChain::Union{Flux.Dense, Flux.Chain} = Flux.Chain(); pooling::String = "sum")
...
Flux.@functor HeterogeneousFFCPNNv4

function (nn::HeterogeneousFFCPNNv4)(states::BatchedHeterogeneousTrajectoryState)
...
function (nn::HeterogeneousFFCPNNv4)(states::HeterogeneousTrajectoryState)
    variableIdx = states.variableIdx
    actionSpaceSize = size(states.fg.valnf, 2)
